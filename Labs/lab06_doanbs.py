...
def has_sublist(s, sublist):
    ...
    False
    """
    sublist_temp = sublist
    while s != empty and sublist_temp != empty:
        if(first(s) != first(sublist_temp)):
            sublist_temp = sublist
        else:
            sublist_temp = rest(sublist_temp)
        s = rest(s)
    return sublist_temp == empty
...
def has_kitty_gene(dna):
    ...
    catcat_temp = catcat
    while dna != empty and catcat_temp != empty:
        if(first(dna) != first(catcat_temp)):
            catcat_temp = catcat
        else:
            catcat_temp = rest(catcat_temp)
        dna = rest(dna)
    return catcat_temp == empty

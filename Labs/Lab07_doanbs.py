...
class Player(object):
    def __init__(self, name, place):  # Constructor
    ...
    def go_to(self, direction):
        ...
        """
        if type(direction) != str:  # Check for valid input
            print("Direction has to be a string.")
        elif direction in self.place.exits:  # If player can go to direction
            self.place = self.place.exit_to(direction)  # Go to direction
        else:  # If player can't go to direction
    ...
    def talk_to(self, person):
        ...
        Albert is not here.
        """
        if type(person) != str:  # Check for valid input
            print("Person has to be a string.")
        elif person in self.place.characters:  # If person is at the current place
            print(self.place.characters[person].talk())  # Talk to person
        else:  # If person is not at the current place
    ...
    def take(self, thing):
        ...
        Hotdog is not here.
        """
        if type(thing) != str:  # Check for valid input
            print("Thing should be a string.")
        elif thing in self.place.things:  # If thing is at the current place
            print("You take the " + thing)
            self.backpack.append(self.place.take(thing))  # Take thing
        else:  # If thing is not at the current place
...
randy = Character(
    "Randy",
    "I can't believe I lost my wallet again! " "I wish someone could find it for me.",
)
albert = Character("Albert", "Randy went to DAAP for potluck. You can find her there.")
yulin = Character(
    "Yulin", "Are you going to potluck? " "You should bring some board games!"
)
jeffrey = Character(
    "Jeffrey",
...
derrick = Character(
    "Derrick", "I heard you like board games. " "Have you gone to Gamestop?"
)
ken = Character("Ken", "Hey! Want to play ultimate frisbee?")
student = Character(
    "Student", "I once went into TUC and got lost for 3 days! " "That place is a maze!"
)
...
# Places:
tuc = Place(
    "Tangeman University Center", "You are at Tangeman University Center", [], []
)
...
me = Player("Burgy", tuc)  # Use constructor to set player's name and starting place
